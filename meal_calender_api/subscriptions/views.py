...
class CreateCheckoutSessionView(APIView):
	...
	def post(self, request):
		...
		try:
			...
			return Response({"code": "invalid_plan", "message": "Invalid or inactive plan"}, status=400)
...
class StripeWebhookView(APIView):
	...
	def post(self, request):
		...
		if not sig_header:
			return Response({"code": "missing_signature", "message": "Missing Stripe-Signature"}, status=400)

		if not settings.STRIPE_WEBHOOK_SECRET:
			return Response({"code": "webhook_not_configured", "message": "Webhook secret is not configured"}, status=503)

		try:
			import stripe
		except ImportError:
			return Response({"code": "stripe_unavailable", "message": "stripe package is not installed"}, status=503)
		...
		try:
			...
			return Response({"code": "invalid_signature", "message": "Invalid signature"}, status=400)
